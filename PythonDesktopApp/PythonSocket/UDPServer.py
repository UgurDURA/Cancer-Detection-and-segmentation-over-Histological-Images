...
while True:
    ...
    SuperResolutionImageHorizontal= np.zeros((480,4050,3), dtype=np.uint8)

    for i in range(12):
        SuperResolutionImageVertical = np.zeros((480,810,3), dtype=np.uint8)
        for j in range(6):
            opencvImage = cv2.cvtColor(np.array(imgReceived), cv2.COLOR_RGB2BGR)
            opencvImage = opencvImage[(i*160):((i*160)+160), (j*180):((j*180)+180)]
            result = sr.upsample(opencvImage)
            if j<7:
                SuperResolutionImageVertical = cv2.hconcat([SuperResolutionImageVertical, result[:,:,::-1]])
                print(SuperResolutionImageVertical.shape)
        if i<13:
            SuperResolutionImageHorizontal = cv2.vconcat([SuperResolutionImageHorizontal, SuperResolutionImageVertical])
            print(SuperResolutionImageHorizontal.shape)
    ...
    cv2.imwrite("SuperResolutionResult.png",SuperResolutionImageHorizontal)
    ...
    class MainWindow(ttk.Frame):
        ...
        def __init__(self, mainframe, path):
            ...
            self.master.title('MicrosPhone')
            #self.master.geometry('800x450+100+10')  # size of the main window
            self.master.geometry('1460x1184+10+10')  # size of the main window
    ...
    filename = "SuperResolutionResult.png" # place path to your image here
